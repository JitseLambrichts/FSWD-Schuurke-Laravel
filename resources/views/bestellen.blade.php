            ...
            <div id="nieuwe-bestellingen">
                    ...
                    @if($winkelwagen)
                        @forelse($items as $item)
                            <div class="winkelwagen-item">
                                <div class="item-info">
                                    <p>{{ $item->gerecht->naam }} (€{{ number_format($item->gerecht->prijs, 2, ',', '.') }}) x {{ $item->aantal }}</p> <!-- hier weer overal getal omzetten naar prijs -->
                                    <p>Subtotaal: <strong>€{{ number_format($item->gerecht->prijs * $item->aantal, 2, ',', '.') }}</strong></p>
                                </div>
                                <div class="verwijderen-container"> <!-- eerst item-actions -->
                ...
                @if($winkelwagen && $items->count() > 0)
                    <form id="bestel-formulier" action="{{ route('winkelwagen.bestellen') }}" method="POST">
                        ...
                        @error('betaalmethode') <!-- errors laten zien als er iets misloopt -->
                ...
                @forelse ($vorigebestellingen as $vorigebestelling)
                    <div class="vorige-bestelling">
                        <div id="info-links">
                            ...
                            <p>Afhalen: {{ $vorigebestelling->afhaaltijdstip ? date('d/m/Y H:i', strtotime($vorigebestelling->afhaaltijdstip)) : 'Niet opgegeven' }}</p>
